...
type FormStep = 'email' | 'verification' | 'securityQuestion' | 'adminContact' | 'success';
...
const securitySchema = z.object({
  answer: z.string().min(1, { message: "Please provide an answer to the security question" }),
});
...
const ForgotPasswordDialog: React.FC<ForgotPasswordDialogProps> = ({ 
  ...
  const handleEmailSubmit = (data: z.infer<typeof emailSchema>) => {
    ...
    setTimeout(() => {
      // In a real implementation, this would check if the email exists in the database
      // and retrieve security questions if they exist
  ...
  const handleVerificationSubmit = (data: z.infer<typeof verificationSchema>) => {
    ...
    setTimeout(() => {
      // If verification code is correct, show security question
  ...
  const handleSecuritySubmit = (data: z.infer<typeof securitySchema>) => {
    // Simulate API call to check security question answer
    setTimeout(() => {
      // If answer is correct, show success
      // Otherwise show admin contact option
      if (data.answer.toLowerCase() === "fluffy") {
        ...
        toast({
          ...
          description: "You may need to contact your administrator for assistance.",
          variant: "destructive",
        });
      }
    }, 1000);
        ...
        return (
          <div className="space-y-4">
            <div className="text-sm text-gray-600">
              <p className="mb-2">
                You've exhausted the automated recovery options. Please contact your organization's administrator to reset your password.
              </p>
              <div className="bg-gray-50 p-4 rounded-md border border-gray-200 mt-2">
                <p className="font-medium text-sm">Organization Administrator Contact</p>
                <p className="text-sm mt-1">Email: admin@yourorganization.com</p>
      ...
      <DialogContent className="sm:max-w-md bg-white text-black relative">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            {step === 'securityQuestion' && "Answer your security question to verify your identity."}
            {step === 'adminContact' && "Get help from your organization administrator."}
          </DialogDescription>
        </DialogHeader>
        <div className="py-2">
          {renderFormStep()}
